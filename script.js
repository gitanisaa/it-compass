...
const KARIR_ICON = {"Software Engineer":"🚀","Cyber Security":"🛡️","UI/UX Designer":"🎨","DevOps Engineer":"⚡","Data Scientist":"📊"};
const KARIR_COLORS = {"Software Engineer":"#8DB4D6","Cyber Security":"#E8A87C","UI/UX Designer":"#D4A0B8","DevOps Engineer":"#6BB5A0","Data Scientist":"#D6E6F2"};
const NILAI_BOBOT = {'A+':4.0,'A':4.0,'A-':3.7,'B+':3.3,'B':3.0,'B-':2.7,'C+':2.3,'C':2.0,'C-':1.7,'D+':1.3,'D':1.0,'D-':0.7,'E':0.0};
...
function renderDonutChart() {
    ...
    sorted.forEach(([bidang, score]) => {
        ...
        html += `<div class="legend-item"><span class="color-dot" style="background:${color};"></span>${KARIR_ICON[bidang] || '📌'} ${bidang}<span class="percent">${score}%</span></div>`;
    });
    html += `</div>`;
...
function renderRecommendation() {
    ...
    details.innerHTML = `<span class="item">📊 Skor: ${last.skor}%</span><span class="item">📚 ${filled}/${total} MK Diisi</span><span class="item">📅 ${last.date}</span>`;
}
...
function getRecommendations(topBidang, results, empty) {
    const recs = {
        "Software Engineer": ["🎯 Perdalam Framework Backend (Laravel/Express/Django)","🎯 Kuasai Database (SQL & NoSQL)","🎯 Pelajari Microservices & API Design","🎯 Buat Portfolio dengan Project Fullstack"],
        "Cyber Security": ["🔐 Pelajari Ethical Hacking & Pentesting","🔐 Kuasai Network Security & Cryptography","🔐 Ikuti Sertifikasi CEH atau CompTIA Security+","🔐 Praktek di Platform TryHackMe/HackTheBox"],
        "UI/UX Designer": ["🎨 Kuasai Figma / Adobe XD / Sketch","🎨 Pelajari Design System & Design Thinking","🎨 Buat Portfolio dengan Case Study","🎨 Pelajari HTML/CSS dasar untuk implementasi"],
        "DevOps Engineer": ["☁️ Kuasai Docker & Kubernetes","☁️ Pelajari CI/CD (Jenkins/GitHub Actions)","☁️ Kuasai Cloud Platform (AWS/GCP/Azure)","☁️ Pelajari Infrastructure as Code (Terraform)"],
        "Data Scientist": ["📊 Pelajari Python (Pandas, NumPy, Scikit-learn)","📊 Kuasai SQL & Data Visualization","📊 Pelajari Machine Learning & Deep Learning","📊 Ikuti kompetisi di Kaggle"]
    };
    let extra = [];
    if (empty > 0) extra.push(`📌 Lengkapi ${empty} mata kuliah yang masih kosong untuk hasil lebih akurat`);
    const sorted = Object.entries(results).sort((a, b) => a[1] - b[1]);
    const weakest = sorted[0][0];
    const weakScore = sorted[0][1];
    if (weakScore < 60 && weakScore > 0) extra.push(`📌 Tingkatkan skill di ${weakest} (skor ${weakScore}%) untuk karir yang lebih luas`);
